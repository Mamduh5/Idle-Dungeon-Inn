...
export function applyOfflineProgress(state: GameState, now: number): GameState {
  ...
  while (remainingMs > 0 && iterations < OFFLINE_PROGRESS_MAX_ITERATIONS) {
    ...
    if (run.status === "preparing" && preparingStopReason) {
      stoppedReason = preparingStopReason;
      break;
    }
...
function createOfflineReportEvent(state: GameState, createdAt: number, stats: OfflineProgressStats): RecentEvent {
  ...
  if (stats.floorsCleared > 0 || stats.coinsGained > 0) {
    return {
      ...
      type: "offline_report",
      createdAt,
      message: `While you were away for ${awayText}, ${partyName} cleared ${stats.floorsCleared} floor(s) and earned ${stats.coinsGained} coins.${capSuffix}${stopSuffix}`,
      severity: "success",
      partyId: party?.id,
  ...
  return {
    ...
    type: "offline_report",
    createdAt,
    message: `While you were away for ${awayText}, the inn waited.${capSuffix}${stopSuffix}`,
    severity: "info",
    partyId: party?.id,
    floor: state.highestFloorCleared
  };
